...
class NeuralNetwork(object):
    ...
    def load_data(self,N,data_name):
        ...
        return train_input, train_target,valid_input, valid_input,valid_target,test_input,test_target,vocab
    ...
    def fprop(self,input_batch, word_embedding_weights, embed_to_hid_weights,hid_to_output_weights, hid_bias, output_bias):
        ...
        :param input_batch:The input data as a matrix of size numwords X batchsize where,numwords is the number of words, batchsize is the number of data points.So,\
        if input_batch(i, j) = k then the ith word in data point j is wordindex k of the vocabulary.
        :param word_embedding_weights: Word embedding as a matrix of size vocab_size X numhid1, where vocab_size is the size of the vocabulary, numhid1 is the
                                       dimensionality of the embedding space
        :param embed_to_hid_weights:Weights between the word embedding layer and hidden layer as a matrix of size numhid1*numwords X numhid2, numhid2 is the number of hidden units.
        :param hid_to_output_weights:Weights between the hidden layer and output softmax unit as a matrix of size numhid2 X vocab_size
        ...
        embedding_layer_state=word_embedding_weights[input_batch.reshape(1,),:].reshape(numhid1*numwords,)

        # COMPUTE STATE OF HIDDEN LAYER.
        # Compute inputs to hidden units.
        input_to_hidden_units=embed_to_hid_weights*embedding_layer_state+np.matlib.repmat(hid_bias,1,batchsize)
        ...
        # exponents.
        inputs_to_softmax = inputs_to_softmax - np.matlib.repmat(max(inputs_to_softmax), vocab_size, 1)

        # Compute exp.
        output_layer_state = np.exp(inputs_to_softmax)

        #Normalize to get probability distribution.
        output_layer_state = output_layer_state/np.matlib.repmat(sum(output_layer_state, 1), vocab_size, 1)
...
def main():
    ...
    train_input, train_target,valid_input, valid_input,valid_target,test_input,test_target,vocab=nn.load_data(100,"data.mat")
